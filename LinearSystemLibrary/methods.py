...
class LinearSystemSolver:
    ...
    DESCRIZIONE: 
    Funzione che implementa il Metodo di Jacobi: x(k+1) = x(k) + P^(-1) ⋅ (A⋅x(k) - b)
    ...
    def jacobi(self) -> tuple[Any, timedelta, float, int]:
        ...
        for i in range(self.iteration):
            ...
            # Calcolo il passo successivo: x(k+1) = x(k) + P^(-1) ⋅ (A⋅x(k) - b)
            self.x = self.x + self._getReverseP_jacobi().dot(residual)
    ...
    le entrate del vettore x già calcolate.
    ...
    def gauss_seidel(self) -> tuple[Any, timedelta, float, int]:
        ...
        for i in range(self.iteration):
            ...
            for j in range(self.row):
                # Estraggo la riga dalla matrice
                row = self.matrix.getrow(j).toarray()[0]
                # Calcolo il nuovo valore utilizzando quelli vecchi
                self.x[j] = (self.b[j] - np.dot(row[:j], self.x[:j]) - np.dot(row[j + 1:], self.x[j + 1:])) / row[j]
